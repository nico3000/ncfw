...
public:
  ...
  float sin() const { return std::sin(m_rad); }
  float cos() const { return std::cos(m_rad); }
  float tan() const { return std::tan(m_rad); }
...
private:
  ...
  Angle(float p_rad) : m_rad(p_rad - 2.0f * M_PI * std::floor(p_rad / (2.0f * M_PI))) {}
};
